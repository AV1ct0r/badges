...
3. Легкое решение заданий аддона Positive Labs (без написания прошивки USB-I2C адаптера)
   ...
   3.2. В самом начале оперативки видим 2 флага с нечитаемыми символами (flag1, flag2) и ключ

   ![](/offzone2024/40_hex1.png)

   а чуть подальше похожие 96 байт: key и оригинальные строки, из которых получаются флаги (генерируется 32 байта, потом 6 байт в конфе отрезается, а оставшиеся обрамляются flag{ } ).
...
4. ToDo - буду дополнять writeup постепенно
